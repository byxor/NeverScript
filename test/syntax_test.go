...
func TestSyntax(t *testing.T) {
	entries := []struct {
		...
		// Floats ------------------------------------------------------------
		{[]Token{{Float, []byte{any, 0x00, 0x00, 0xA0, 0x40}}}, "5.0000000f"},
		{[]Token{{Float, []byte{any, 0x33, 0x33, 0x03, 0x41}}}, "8.1999998f"},
		{[]Token{{Float, []byte{any, 0x33, 0xB3, 0x05, 0x43}}}, "133.6999969f"},
		{[]Token{{Float, []byte{any, 0x3a, 0xdf, 0x47, 0x40}}}, "3.1229997f"},

		// Pairs ------------------------------------------------------------
		{[]Token{{Pair, []byte{any, 0x66, 0x66, 0x66, 0x3F, 0x00, 0x00, 0x80, 0x3F}}},
			"vec2<0.9000000f, 1.0000000f>"},

		{[]Token{{Pair, []byte{any, 0xCD, 0xCC, 0x4C, 0x3F, 0x00, 0x00, 0x80, 0x3F}}},
			"vec2<0.8000000f, 1.0000000f>"},

		{[]Token{{Pair, []byte{any, 0x00, 0x00, 0xA0, 0x40, 0x00, 0x00, 0xA0, 0x40}}},
			"vec2<5.0000000f, 5.0000000f>"},

		// // Vectors ------------------------------------------------------------
		{[]Token{{Vector, []byte{
			any, 0x66, 0x66, 0x66, 0x3F,
			0x00, 0x00, 0x01, 0x16,
			0xCD, 0xCC, 0x4C, 0x3F}}},
			"vec3<0.9000000f, 0.0000000f, 0.8000000f>"},

		{[]Token{{Vector, []byte{
			any, 0x00, 0x00, 0xC8, 0x42,
			0x00, 0x00, 0x48, 0x42,
			0xCD, 0xCC, 0x4C, 0x3F}}},
			"vec3<100.0000000f, 50.0000000f, 0.8000000f>"},

		{[]Token{{Vector, []byte{
			any, 0x00, 0x00, 0x70, 0x41,
			0x00, 0x00, 0x01, 0x16,
			0x00, 0x00, 0x70, 0x41}}},
			"vec3<15.0000000f, 0.0000000f, 15.0000000f>"},
