...
func TestCompilation(t *testing.T) {
	Convey("NeverScript code compiles to QB bytecode", t, func() {
	...
	Convey("Invalid NeverScript does not compile", t, func() {
		sourceCodeContent := `APLSR{PL!{APSFKKoia::'':;;'#'#F"A:'''SF:ASF"`
		sourceCode := NeverScript.NewSourceCode(sourceCodeContent)

		_, err := compilerService.Compile(sourceCode)
